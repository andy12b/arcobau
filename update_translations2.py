...
def update_file():
    ...
    for lang in ["EN", "FR", "DE", "RO", "IT"]:
        # Match from `services: {` down to its specific closing `}` just before `}, \n expertise: {`
        pattern_home = rf'({lang}:\s*{{[\s\S]*?home:\s*{{[\s\S]*?)(      services: {{(?:[^{{}}]|{{[^{{}}]*}})*}})(\n    }},[\s\S]*?expertise:\s*{{)'
        ...
        # Match expertise services
        pattern_exp = rf'({lang}:\s*{{[\s\S]*?expertise:\s*{{[\s\S]*?)(      services: {{(?:[^{{}}]|{{[^{{}}]*}})*}})(,\n      steps:\s*\[)'
